...
sem_t empti; 
sem_t full; 
pthread_mutex_t prod_consume_lock;
//extra
...
//extra
void consumer_read(int);
void producer_library(int);
//extra
void* staffTask(void* arg)
{
    int stuff_num= *(int *)arg;
    //read(stuff_num);
    consumer_read(stuff_num);
}
...
int main(void)
{
    ...
    //extra
    pthread_mutex_init(&prod_consume_lock, 0);
    sem_init(&empti,0,n/m);
    sem_init(&full,0,0);
    //extra
...
//extra
void producer_library(int s_num)
{
    sem_wait(&empti);
    pthread_mutex_lock(&prod_consume_lock);
    ...
    pthread_mutex_unlock(&prod_consume_lock);
    sem_post(&full);
}
void consumer_read(int stuff_num)
{
    sem_wait(&full);
    pthread_mutex_lock(&prod_consume_lock);
    pthread_mutex_lock(&locked);
    ...
    sleep(y);
    pthread_mutex_lock(&locked);
    cout<<"Staff "<<stuff_num+1<<" has finished reading the entry book at time "<<time(NULL)-initial_time<<". No. of submission = "<<subCount<<endl;
    pthread_mutex_unlock(&locked);
